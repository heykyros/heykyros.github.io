...
function runGame(){
    ...
    const pathPlatform = new Sprite ({image: battlePlatform, position: {x: 585, y: 120}, scale:2})
    const playerPathPlatform = new Sprite ({image: playerBattlePlatform, position: {x: -255, y: 377}, scale:2})
    const battleDialogueBox =  new Sprite ({image: battleDialogue, position: {x: 0, y: (canvas.height - 213)}, scale: 0.463})
    const monaLisa2 = new Sprite({image: painting2, position: {x: 750, y: -5}, scale:0.5})
